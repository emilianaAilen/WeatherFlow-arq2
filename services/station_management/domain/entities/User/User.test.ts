...
describe('User', () => {
  ...
  describe('getSubscriptions', () => {
    ...
    it('returns the provided subscriptions list', () => {
      const subscriptions = SubscriptionsList.create(['station-1', 'station-2']);
      const user = User.create('id-1', 'Jane', 'Doe', 'jane@example.com', subscriptions);
      expect(user.getSubscriptions().stationIds).toEqual(['station-1', 'station-2']);
    });
  });
