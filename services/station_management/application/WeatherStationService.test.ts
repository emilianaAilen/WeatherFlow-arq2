...
describe("WeatherStationService", () => {
  ...
  it("should create a weather station and publish event", async () => {
    ...
    expect(stationEventPublisher.publishStationCreated).toHaveBeenCalledWith(
      result,
    );
  ...
  it("should throw 409 if name already taken on create", async () => {
    ...
    weatherStationRepository.findStationByName.mockResolvedValue(
      existingStation,
    );
  ...
  it("should update a weather station and publish event", async () => {
    ...
    expect(stationEventPublisher.publishStationUpdated).toHaveBeenCalledWith(
      result,
    );
  ...
  it("should delete a weather station and publish event", async () => {
    ...
    expect(stationEventPublisher.publishStationDeleted).toHaveBeenCalledWith(
      "station-id",
    );
  });
  ...
  it("should return a station when searching by existing name", async () => {
    ...
    expect(weatherStationRepository.findStationByName).toHaveBeenCalledWith(
      "Station Alpha",
    );
  ...
  it("should throw 404 if station not found on delete", async () => {
    weatherStationRepository.findById.mockResolvedValue(null);
    await expect(service.deleteStation("station-id")).rejects.toThrow(
